...
def gaze(frame, points):
    ...
    if transformation is not None:  # if estimateAffine3D secsseded
        ...
        if left_pupil is not None and eye_pupil2D is not None:
            eye_center = left_pupil  # (x, y) format
            gaze_point = eye_pupil2D[0][0].ravel()  # Flatten and convert to (x, y) format
            # Calculate the 2D vector from the eye center to the gaze point
            gaze_vector = np.array(gaze_point) - np.array(eye_center)
            
            # Calculate yaw as the angle in the horizontal plane
            yaw_rad = np.arctan2(gaze_vector[0], -gaze_vector[1])  # Note: Inverted y-axis in image coordinates
            yaw_deg = np.degrees(yaw_rad)
            
            # The pitch calculation remains a challenge without Z-axis information; 
            # this simplistic method assumes a direct correlation in the 2D plane, which is imprecise
            # For demonstration purposes only:
            pitch_deg = np.degrees(np.arcsin(gaze_vector[1] / np.linalg.norm(gaze_vector)))
        ...
        # cv2.putText(frame,str(f"Pitch:{x:.2f}"),(10,20), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,255),1)
        # cv2.putText(frame,str(f"Yaw:{y:.2f}"),(10,40), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,255),1)
        # cv2.putText(frame,str(f"Roll:{z:.2f}"),(10,60), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,255),1)
